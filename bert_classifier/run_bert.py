...
from pybert.model.nn.bert_for_multi_class import BertForMultiClass
from pybert.callback.modelcheckpoint import ModelCheckpoint
from pybert.callback.trainingmonitor import TrainingMonitor
from pybert.train.metrics import F1Score, ClassReport
...
def run_train(args):
    ...
    train_data, type = processor.get_train(config['data_dir'] / f"{args.data_name}.train.csv")
    ...
    valid_data,type = processor.get_dev(config['data_dir'] / f"{args.data_name}.valid.csv")
...
def main():
    ...
    parser.add_argument('--data_name', default='cnews', type=str)
    ...
    # 10 k folder
    parser.add_argument("--k_folder", default=0, type=int)
    ...
    parser.add_argument("--0.01", default=1e-8, type=float)
    ...
    # if True:
        ...
        targets, sentences = data.read_data(raw_data_path=config['raw_data_path'],
                                            preprocessor=EnglishPreProcessor(stopwords_path="pybert/dataset/greeting_word.txt"),
                                            is_train=True,label2id=label2id)
        data.train_val_split(X=sentences, y=targets, shuffle=True, stratify=targets,
                             valid_size=args.k_folder, data_dir=config['data_dir'],
                             data_name=args.data_name,seed=77889900)
    if args.do_train:
    # if True:
        run_train(args)
